...
export default function AuxServicesSection() {
  ...
  useEffect(() => {
    ...
    // Increased to 2500ms for a better "reading" pace. 
    // If you want it faster, 2000ms is the sweet spot.
    const interval = setInterval(() => {
      setIdx((prev) => (prev + 1) % SLIDES.length);
    }, 2500); 

    return () => clearInterval(interval);
  }, [paused, idx]); // Added idx to dependencies to ensure timer reset on manual click

  const s = SLIDES[idx];

  return (
    <section className="bg-white text-neutral-900 py-8 sm:py-10 md:py-16" id="training">
      <div className="mx-auto w-full max-w-6xl px-4 sm:px-6 md:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-7">
            ...
            <p className="mt-4 text-sm sm:text-md font-semibold text-neutral-500 uppercase tracking-wide">
              Services Offered
            </p>
          ...
          {/* IMAGE CONTAINER - Pause only happens here now */}
          <div 
